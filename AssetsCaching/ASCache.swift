...
public class ASCache: NSObject {
    public static let shared = ASCache()
    ...
    public var memoryCostLimit: Int = 60 {
        willSet {
            memory.costLimit = newValue * 1024 * 1024
        }
    }
    
    public var diskCostLimit: Int = 60 {
        willSet {
            disk.costLimit = newValue * 1024 * 1024
        }
    }
    
    override init() {
        memory = ASMemoryCache()
        memory.costLimit = memoryCostLimit * 1024 * 1024
        disk = ASDiskCache()
        disk.costLimit = diskCostLimit * 1024 * 1024
