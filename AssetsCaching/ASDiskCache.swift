...
final class ASDiskCache: NSObject {
    ...
    func object(key: String) -> Any? {
        let path = cacheURL.appendingPathComponent(trim(key))
    ...
    private func deleteObjectsByDate() {
        let block = BlockOperation { [weak self] in
            ...
            for dict in dates {
                do {
                    let path = self.cacheURL.appendingPathComponent(trim(dict.key))
    ...
    private func writeToDisk(_ obj: Any, key: String) {
        ...
        let block = BlockOperation { [weak self] in
            ...
            do {
                ...
                let path = self.cacheURL.appendingPathComponent(self.trim(key))
    ...
    private func trim(_ key: String) -> String {
        return key
